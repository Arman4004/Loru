...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        print '\n\n [%s!%s] ops you got no result :('%(M,N);exit()
...
def moch_yayan():
    ...
    if pepek == '':
        ...
        try:
            ...
            print '\n [ the result of the crack stored in your file ]\n'
            for file in dirs:
            ...
            print("\n %s[%s×%s] ops you got no result :("%(N,M,N))
            raw_input('\n  [ %sKEMBALI%s ] '%(O,N));moch_yayan()
    elif pepek in['8','08']:
        seting_yntkts()
    elif pepek in['9','09']:
...
def cek_ingfo(kontol):
    try:
        user = raw_input("\n [%s+%s] enter id or username
Enter your id or username: "%(O,N))
        if user == '':
